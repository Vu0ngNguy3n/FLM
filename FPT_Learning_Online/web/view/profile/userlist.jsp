...
<html>
    ...
    <body class="ttr-opened-sidebar ttr-pinned-sidebar">
        ...
        <main class="ttr-wrapper">
            <div class="db-breadcrumb" style="display: flex; justify-content: center;">
                <h4 >User List Management</h4>
            </div>	
            <div class="mb-4 ml-5">
                <form action="${pageContext.request.contextPath}/FLM/userlist" method="post">                                      
                    <div class="input-group input-group-sm">
                        <table class="auto-style1 mb-3">
                            <tbody><tr>
                                    <th class="auto-style13" style="text-align: right">Search by User:                                     
                                    </th>                  
                                    <td class="auto-style10">
                                        <div class="form-inline">
                                            <div class="input-group">
                                                <span class="input-group-btn">
                                                    <input  type="text" class="form-control" aria-label="Small" aria-describedby="inputGroup-sizing-sm" placeholder="Enter Username..." name="userSearch" style=width:500px;>
                                                </span>                                                
            ...
            <div class="container">
                <c:if test="${requestScope.update ne null }">
                    <input type="hidden" id="show" value="${requestScope.update}">
                </c:if>
                <div id="success-toast" class="toast hide">
                    ...
                    <div class="toast-body">
                        <c:if test="${requestScope.update eq true}">
                            <i class="fa-solid fa-lock"></i> User ${requestScope.userCheck} is Inactive successfully!!!
                        </c:if>
                        <c:if test="${requestScope.update eq false}">
                            <i class="fa-solid fa-lock-open"></i> User ${requestScope.userCheck} is Active successfully!!!
                        </c:if>

                    </div>
                </div>
                <c:if test="${requestScope.isNull eq 0}">
                    <h5 style="color: orange;">No User Can Be Found!!!</h5>
                </c:if>
                <c:if test="${requestScope.pages ne 0}">
                    <h5 style="color: orange;">Have ${requestScope.pages} users can found!!</h5>
                </c:if>
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="table-responsive">
                                <table class="table no-wrap user-table mb-0">
                                    ...
                                    <tbody id="content">
                                        <c:forEach items="${requestScope.accounts}" var="a" varStatus="index" >
                                            <c:if test="${account.username ne a.username}">
                                                <tr class="numrow">
                                                    <td class="pl-4">${index.index+1}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <span class="text-muted">${a.gmail}</span><br />
                                                        <span class="text-muted">${a.phone}</span>
                                                    </td>
                                                    <td>
                                                        <span class="text-muted">${a.getRole().rolename}</span><br />
                    ...
                    <nav aria-label="..." class="ml-3">
                        <ul class="pagination">
                            <c:forEach var="i" end="${requestScope.pages/6}" begin="0">
                                <li class="page-item"><a class="page-link" onClick="LoadMoreList(${i})">${i+1}</a></li>
                                </c:forEach>
                        </ul>
                    </nav>
                </div>
    ...
    <script>
                                    ...
                                    var status = document.getElementById('show').value;
                                    if (status !== null) {
                                        showToast();

                                    }
                                    function LoadMoreList(index) {
                                        $.ajax({
                                            url: "${pageContext.request.contextPath}/FLM/loadmore",
                                            type: 'GET',
                                            data: {
                                                num: index,
                                                name: '${a.username}',
                                                userSearch: '${requestScope.userSearch}'
                                            },
